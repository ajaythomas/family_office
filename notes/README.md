...
1. Install uv
1. Copy pyproject.toml + uv.lock and install dependencies
1. Copy the app source code
1. Define the startup command (alembic upgrade head && fastapi run)
...
1. Postgres → runs in Docker (docker compose up -d)
1. FastAPI API → runs directly on your Mac via uv run fastapi dev
1. React frontend → runs directly on your Mac via npm run dev
...
1. Smaller, cleaner images — COPY . . in the Dockerfile won't accidentally pull in dev-only files, secrets (.env), or cache
  directories
...
1. Sometimes, it jumps to wrong conclusions:
    ...
    1. When I asked it to update the gain/loss column to be percentage instead of absolute dollar amount, it made the change in FE vs in the portfolios.py class' method get_portfolio(). BE changes always preferred so if a mobile client later introduced, it gets that too.
    1. When working on cedar_authz.py, it saw cedarpy.isauthorized() fail because of type errors and assumed it was broken on Python 3.14. But, cedarpy docs clearly says 3.14 is supported. It tried to downgrade my Python version (insane!)
        1. After questioning it, it agreed, it over concluded.
        1. I asked it to look at principal/action/resource triplet it was sending to that method and try different options. Finally, it worked 
...
1. Claude Code security review caught a few holes:
    ...
    1. JWT token passed in URL query string (app/routers/auth.py:62) `GET /auth/google-calendar?token=<24h_jwt>`
    ...
    1. JWT-in-URL fix (app/routers/auth.py, web/src/lib/api.ts, web/src/App.tsx):
